...
class CYSpriter {
  ...
  static makeSpritePath(sprite, dir) {
    ...
    var ext = '.png';
    return path.join(dir, sprite.selector.replace(/^(\.)|(#)/, '') + ext);
  }
  ...
  static save(sprite, options, sprites) {
    ...
    return Q.Promise(function(resolve, reject) {
      ...
      return Q.nfcall(fs.writeFile,
        CYSpriter.makeSpritePath(sprite, options.dest),
  ...
  static writeRules(sprite, options, css) {
    css.walkRules(sprite.name, (rule) => {
      ...
      sprite.imageList.forEach((image) => {
        ...
        css.append(
          postcss
            ...
            .append(postcss.decl({
              prop : "width",
              value : width > 1 ? `${width}px` : width
            }))
            .append(postcss.decl({
              prop : "height",
              value : height > 1 ? `${height}px` : height
            }))
            .append(postcss.decl({
              prop : "background-position",
              value: (posX > 1 ? `${posX}px` : posX) + ' ' + (posY > 1 ? `${posY}px` : posY)
            }))
        )
        ;
      });
      ...
      rule
        ...
        .append(postcss.decl({
          ...
          value : 'url(' + CYSpriter.makeSpritePath(sprite, options.dest) + ')'
        }))
      ;
