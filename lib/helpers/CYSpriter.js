...
import { calcImagePosition, SPRITE_LAYOUT, logger, isTruthy } from './utils';
...
class CYSpriter {
    ...
    css.walkRules(sprite.originalSelector, (rule) => {
      ...
      sprite.imageList.forEach((image) => {
        ...
        let
          ...
          position = calcImagePosition(image.position, sprite.layout)
          ;

        image.cssPosition = position;
    ...
    return Q.Promise((resolve, reject) => {
      ...
      css.walkRules(configs.NAMESPACE_REGEX, rule => {
        ...
        if(!CYSpriter.hasMandatoryDeclarations(rule)) {
          logger.warn(`missing cy-src propery in ${rule.selector} - `, rule.source.start);
          return;
        }
  ...
  static resolveIncludes(requirements, rule, sprites, css) {
    let [selector, includeSize] = requirements;
    includeSize = isTruthy(includeSize);

    sprites.forEach((sprite, i) => {
      sprite.imageList.forEach((image, j) => {
        if(image.selector !== selector) {
          return;
        }

        image.sprite.rule.selector += `,\n${rule.selector}`;
        rule.selector += `,\n${image.selector}`;

        rule.append(
          postcss.decl({
            prop: 'background-position',
            value: `${image.cssPosition.x} ${image.cssPosition.y}`
          })
        );
        ...
        if(includeSize) {
          rule.append(
            postcss.decl({
              prop: 'width',
              value: image.size.width !== 0 ? `${image.size.width}px` : image.size.width
            }),
            postcss.decl({
              prop: 'height',
              value: image.size.height !== 0 ? `${image.size.height}px` : image.size.height
            })
          );
        }
