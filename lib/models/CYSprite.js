...
class CYSprite {
  ...
  static mergeOptions(rule, options) {
    ...
    rule.walkDecls(new RegExp(configs.NAMESPACE), function(decl) {
      ...
      switch(prop) {
        case 'padding':
          let padding = parseFloat(decl.value);
          isNaN(padding) || (_options.padding = padding);
          break;
        case 'retina':
          _options.retina = isTruthy(decl.value);
          break;

        case 'include-size':
        case 'includesize':
          _options.includeSize = isTruthy(decl.value);
          break;

        default:
          console.log('MergeOptions:Switch:default', {[prop]: decl.value});
      }
